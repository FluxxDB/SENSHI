...
local function preRenderCheck()
    ...
    if 
      getMagnitude(origin, currentOrigin) > 1
    then
        ...
        --         if not entity:IsA("Model") or not entity:FindFirstChildOfClass("Humanoid") then continue end
...
function RenderController:onInit()
end
