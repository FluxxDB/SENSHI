...
local function cameraUpdate(delta)
	...
	local xControl = Vector2.new(math.sin(math.rad(mousePosition.X)), math.cos(math.rad(mousePosition.X)))
		* cameraDistance
	...
	local cameraObstructor = workspace:Spherecast(
		cameraSubject,
		CAMERA_RADIUS,
		CFrame.new(cameraSubject, cameraMount).LookVector * cameraDistance,
		SPHERECAST_PARAMS
	)

	if cameraObstructor then
		cameraDistanceGoal = math.clamp(
			(cameraObstructor.Distance < cameraDistance) and cameraObstructor.Distance or cameraDistance,
			0.1,
			MAX_CAMERA_DISTANCE
		)
	end
...
local function cameraScroll(inputObject, gameProcessed)
	...
	cameraDistanceGoal = math.clamp(
		(inputObject.Position.Z == 1) and cameraDistance - 2 or cameraDistance + 2,
		MIN_CAMERA_DISTANCE,
		MAX_CAMERA_DISTANCE
	)
end
