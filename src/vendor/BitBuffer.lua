...
-- Potato Useless Changes
function BitBuffer:WriteInt8(int: number): ()
	WriteToBuffer(self, 8, int % (bit32.lshift(1, 8 - 1) * 2))
end

function BitBuffer:WriteUInt8(uint: number): ()
	WriteToBuffer(self, 8, uint)
end

function BitBuffer:ReadInt8(): number
	local value = ReadFromBuffer(self, 8)
	local max = bit32.lshift(1, 8)
	return if value >= max / 2 then value - max else value
end

function BitBuffer:ReadUInt8(): number
	return ReadFromBuffer(self, 8)
end

function BitBuffer:WriteInt16(int: number): ()
	WriteToBuffer(self, 16, int % (bit32.lshift(1, 16 - 1) * 2))
end

function BitBuffer:WriteUInt16(uint: number): ()
	WriteToBuffer(self, 16, uint)
end

function BitBuffer:ReadInt16(): number
	local value = ReadFromBuffer(self, 16)
	local max = bit32.lshift(1, 16)
	return if value >= max / 2 then value - max else value
end

function BitBuffer:ReadUInt16(): number
	return ReadFromBuffer(self, 16)
end

function BitBuffer:WriteInt32(int: number): ()
	WriteToBuffer(self, 32, int % (bit32.lshift(1, 32 - 1) * 2))
end

function BitBuffer:WriteUInt32(uint: number): ()
	WriteToBuffer(self, 32, uint)
end

function BitBuffer:ReadInt32(): number
	local value = ReadFromBuffer(self, 32)
	local max = 0xFFFFFFFF
	return if value >= max / 2 then value - max else value
end

function BitBuffer:ReadUInt32(): number
	return ReadFromBuffer(self, 32)
end

function BitBuffer:WriteUInt64(uint: number): ()
	WriteToBuffer(self, 32, math.floor(uint / (2 ^ 32)))
	WriteToBuffer(self, 32, bit32.band(uint, (2 ^ 32) - 1))
end

function BitBuffer:ReadUInt64(): number
	return (ReadFromBuffer(self, 32) * (2 ^ 32)) + ReadFromBuffer(self, 32)
end

export type BitBuffer = typeof(BitBuffer.new())
return BitBuffer :: BitBuffer